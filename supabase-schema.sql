...
-- ============================================================
-- LYNK.ID SALES SHEET -> AUTO-GRANT (migration 010)
-- ============================================================

-- Every row Apps Script successfully processes gets one entry here
-- (idempotency guard against reprocessing the same sale twice).
...
-- service_role-only: grants paid access, so it must never be
-- callable with an authenticated user's own JWT.
