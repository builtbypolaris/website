/**
 * Lynk.id Financial Tracker sales sheet -> Supabase auto-grant.
 *
 * This script is bound to the Google Sheet Lynk.id writes ONLY
 * Financial Tracker sales into. To-Do List has its own separate sheet
 * and its own copy of this script (see ../todo/Code.gs) — each sheet
 * only ever contains rows for one product, so there's no need to look
 * up which tracker a row is for; TRACKER_ID below is fixed.
 *
 * Confirmed against a real row from the sheet: Lynk.id's own columns
 * include "Status" (order status: SUCCESS/PENDING/etc — do NOT reuse
 * this name for our own tracking column, see CONFIG below) and
 * "Buyer Email" (the buyer's account email).
 *
 * Setup (one-time, in your Google account — not deployable from this repo):
 *   1. Open the Financial Tracker sales sheet.
