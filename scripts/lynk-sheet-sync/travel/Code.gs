/**
 * Lynk.id Travel Planner sales sheet -> Supabase auto-grant.
 *
 * This script is bound to the Google Sheet Lynk.id writes ONLY Travel
 * Planner sales into. Financial Tracker, To-Do List, and Mood Tracker
 * each have their own separate sheet and their own copy of this script
 * (see ../financial/Code.gs, ../todo/Code.gs, ../mood/Code.gs) — each
 ...
 * against a real row once Travel has its first sale, same as the note
 * in ../todo/Code.gs.
 *
 * Setup (one-time, in your Google account — not deployable from this repo):
 *   1. Open the Travel Planner sales sheet.
