/**
 * Lynk.id Weight Loss Tracker sales sheet -> Supabase auto-grant.
 *
 * This script is bound to the Google Sheet Lynk.id writes ONLY Weight
 * Loss Tracker sales into. Financial Tracker, To-Do List, Mood Tracker,
 * and Travel Planner each have their own separate sheet and their own
 * copy of this script (see ../financial/Code.gs, ../todo/Code.gs,
 * ../mood/Code.gs, ../travel/Code.gs) — each sheet only ever contains
 * rows for one product, so there's no need to look up which tracker a
 * row is for; TRACKER_ID below is fixed.
 *
 * Note: TRACKER_ID is 'health' — the Weight Loss Tracker is the
 * reframed Health & Meals tracker (same underlying id/route/data),
 * not a separate product internally.
 *
 * Column names assumed to match the other sheets' layout (Lynk.id's
 * own "Status" order-status column and "Buyer Email" column) — confirm
 * against a real row once Weight Loss has its first sale, same as the
 * note in ../todo/Code.gs.
 *
 * Setup (one-time, in your Google account — not deployable from this repo):
 *   1. Open the Weight Loss Tracker sales sheet.
