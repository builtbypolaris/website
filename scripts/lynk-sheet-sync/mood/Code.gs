/**
 * Lynk.id Mood Tracker sales sheet -> Supabase auto-grant.
 *
 * This script is bound to the Google Sheet Lynk.id writes ONLY Mood
 * Tracker sales into. Financial Tracker and To-Do List each have their
 * own separate sheet and their own copy of this script (see
 * ../financial/Code.gs and ../todo/Code.gs) — each sheet only ever
 * contains rows for one product, so there's no need to look up which
 * tracker a row is for; TRACKER_ID below is fixed.
 *
 * Column names assumed to match the Financial/To-Do sheets' layout
 * (Lynk.id's own "Status" order-status column and "Buyer Email"
 * column) — confirm against a real row once Mood has its first sale,
 * same as the note in ../todo/Code.gs.
 *
 * Setup (one-time, in your Google account — not deployable from this repo):
 *   1. Open the Mood Tracker sales sheet.
